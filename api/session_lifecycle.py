...
def unregister_agent(session_id: str) -> None:
    ...
    with _condition:
        ...
        if entry is not None:
            entry["agent"] = None
            # Remove fully-settled entries to prevent unbounded dict growth.
            # Dirty segments are preserved by the check: if in-flight work remains
            # the entry stays so retries can find it.
            if (entry.get("generation", 0) <= entry.get("committed_generation", 0)
                    and not entry.get("segments")):
