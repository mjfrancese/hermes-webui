...
# Metadata-only session-poll cache (patch-2026-05-23-metadata-poll-cache).
# Avoids reloading session JSON + state.db on every 5-second browser poll.
_METADATA_POLL_CACHE: dict = {}  # sid -> (cached_at, message_count, last_message_at)
_METADATA_POLL_CACHE_TTL = 30  # seconds; at 5s poll = 5 hits per miss
_METADATA_POLL_CACHE_LOCK = threading.Lock()
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/session":
        ...
        try:
            ...
            _meta_cache_hit = None  # patch-2026-05-23-metadata-poll-cache
            ...
            if is_messaging_session:
                ...
                # patch-2026-05-23-metadata-poll-cache: skip expensive JSON + DB
                # reload when a fresh cached result exists.
                with _METADATA_POLL_CACHE_LOCK:
                    _mc = _METADATA_POLL_CACHE.get(sid)
                    if _mc and (time.time() - _mc[0]) < _METADATA_POLL_CACHE_TTL:
                        _meta_cache_hit = _mc
                if _meta_cache_hit is None:
                    ...
                    del sidecar_metadata_session  # allow immediate GC
            _t2 = _time.monotonic()
            effective_model = (
            ...
            if not load_messages:
                # patch-2026-05-23-metadata-poll-cache
                if _meta_cache_hit:
                    _summary_message_count = _meta_cache_hit[1]
                    _summary_last_message_at = _meta_cache_hit[2]
                else:
                    ...
                    with _METADATA_POLL_CACHE_LOCK:
                        _METADATA_POLL_CACHE[sid] = (
                            time.time(), _summary_message_count, _summary_last_message_at
                        )
            else:
